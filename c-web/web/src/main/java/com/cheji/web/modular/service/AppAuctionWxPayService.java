...
public class AppAuctionWxPayService {
    ...
    public JSONObject createBailOrder(JSONObject result, Long carId, HttpServletRequest httpServletRequest, TokenPojo currentLoginUser) {
        ...
        if (auction.getCarBondAmt() == null) {
            auction.getPrice().divide(new BigDecimal(100)).multiply(new BigDecimal(bail));
        } else {
            payAmount = auction.getCarBondAmt();
        }
    ...
    public JSONObject createOnePriceOrder(TokenPojo currentLoginUser, Long carId, HttpServletRequest httpServletRequest) {
        ...
        try {
            ...
            if (appAuction.getSourceType() == 1 && !"1".equals(appAuction.getFixedPrice())) {//是否是一口价,0否,1是
                result.put("code", 401);
            ...
            if (StringUtils.isNotBlank(onePriceLock)) {
                result.put("code", 300);
                result.put("msg", "当前车辆正在被其他人支付，请等待五分钟后重试");
                return result;
            }
...
//            }
            ...
            //如果没有设置单车保证金金额  按照拍卖价的百分之5计算
            BigDecimal payAmount = new BigDecimal(0);
            if (appAuction.getCarBondAmt() == null) {
                appAuction.getPrice().divide(new BigDecimal(100)).multiply(new BigDecimal(bail));
            } else {
                payAmount = appAuction.getCarBondAmt();
            }

            result = wxPay("车己汽车-一口价车辆订金", httpServletRequest, payAmount, appAuction.getId(), result, currentLoginUser, null, "onePriceNotify", AppAuctionWxPayConstant.WX_PAY_STATE_ONE_PRICE);
    ...
    public JSONObject getInfo(Long carId, Integer id) {
        ...
        if (Objects.nonNull(auction) && Objects.nonNull(auction.getPrice())) {
            object.put("bail", auction.getCarBondAmt());
