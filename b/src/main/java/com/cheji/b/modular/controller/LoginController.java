...
import com.cheji.b.modular.service.HuaweiSmsService;
...
public class LoginController extends BaseController {
    ...
    private void setToken(String thirdSessionKey, TokenPojo tokenPojo) {
        stringRedisTemplate.opsForValue().set(RedisConstant.USER_B_TOKEN + thirdSessionKey, JSON.toJSONString(tokenPojo), 60 * 60 * 24 * 30, TimeUnit.SECONDS);
        //记录用户的token和id之间的关系，用于踢下线
        stringRedisTemplate.opsForValue().set(RedisConstant.USER_B_ID_TOKEN + tokenPojo.getAppUserEntity().getId(), thirdSessionKey);
    }
    ...
    private void dissOldUser(int userId) {
        String s = stringRedisTemplate.opsForValue().get(RedisConstant.USER_B_ID_TOKEN + userId);
        if (StringUtils.isNotEmpty(s)) {
            stringRedisTemplate.delete(RedisConstant.USER_B_TOKEN + s);
        }
    }
    ...
    public JSONObject logOutCode(@RequestBody JSONObject in) {
        ...
        String phone = in.getString("phone");
        String code = in.getString("code");
        String systemCode = stringRedisTemplate.opsForValue().get(RedisConstant.USER_REGISTER_PHONE_CODE + phone);
