...
import com.cheji.web.constant.AppAuctionRedisConstans;
...
//            appAuctionEntity.setSelfState(setAsSelfState(appAuctionEntity));
            ...
            if (appAuctionUpEntity != null) {
                if (appAuctionUpEntity.getBeginTime() != null) {
                    appAuctionEntity.setBeginTime(appAuctionUpEntity.getBeginTime());
                    appAuctionEntity.setEndTime(appAuctionUpEntity.getEndTime());
                }
...
//            appAuctionEntity.setSelfState(setAsSelfState(appAuctionEntity));
    ...
    public JSONArray auction2Dto(JSONArray objects, List<AppAuctionEntity> appAuctionEntities, Long userId) {
        ...
        appAuctionEntities.stream().forEach(a -> {
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("id", a.getId().toString());
            if (a.getBrand().length() > 13) {
                jsonObject.put("brand", a.getBrand().substring(0, 13));
            } else {
                jsonObject.put("brand", a.getBrand());
            }
            jsonObject.put("register_date", a.getRegisterDate());
            jsonObject.put("insuredAmount", a.getInsuredAmount());
            jsonObject.put("accidentType", a.getAccidentType());
            jsonObject.put("parkingPlace", a.getParkingPlace());
            jsonObject.put("fixedPrice", a.getFixedPrice());
            jsonObject.put("price", a.getPrice());
            jsonObject.put("insurance", a.getInsurance());
            jsonObject.put("carState", a.getCarState());
            if (Objects.nonNull(userId)) {
                jsonObject.put("collect", appAuctionCollectService.isCollect(userId, a.getId()));
            } else {
                jsonObject.put("collect", null);
            }
            EntityWrapper<AppAuctionUpEntity> wrapper2 = new EntityWrapper<>();
            wrapper2.eq("car_id", a.getId());
            ...
            List<AppAuctionImgEntity> carImgList = appAuctionImgService.selectByCarId(a.getId());
