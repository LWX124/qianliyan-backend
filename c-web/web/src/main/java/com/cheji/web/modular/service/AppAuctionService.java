...
public class AppAuctionService extends ServiceImpl<AppAuctionMapper, AppAuctionEntity> implements IService<AppAuctionEntity> {
    ...
    public JSONObject updateAuctionCar(AppAuctionEntity auctionEntity) throws ParseException {
        ...
        if (auction1.getCarState() >= AppAuctionConstant.THREE || auction1.getCarState() == AppAuctionConstant.ONE) {
            result.put("code", 501);
...
//            log.info("### 热门推荐数据###  city={};hot={}; collect1={}; appAuctionEntities={} ", city, hot, collect1, appAuctionEntities);
    ...
    public JSONObject detail2(Long carId, Long userId) {
        ...
        if (carId != null && carId > 0) {
            ...
            if (appAuctionEntity.getSourceType() == 1) {//信息来源,1,拍卖车，2,二手车
                //浏览次数
                ...
                redisTemplate.opsForValue().increment(AuctionConstant.OLD_CAR_BROWSE_NUM + appAuctionEntity.getId());
                Object num = redisTemplate.opsForValue().get(AuctionConstant.OLD_CAR_BROWSE_NUM + appAuctionEntity.getId());
                appAuctionEntity.setBrowseNum(Long.valueOf(String.valueOf(num)));
            }
            ...
            if (userId != null && appAuctionEntity.getCarState() == 7 && appAuctionEntity.getSourceType() == 1) { //记录登录用户，出价之后需要发送长连接告知用户最新价格
                //计算redis过期时间
...
//                    //如果没有设置保证金金额  按照拍卖价的百分之5计算
//                    BigDecimal payAmount = new BigDecimal(0);
//                    if (appAuctionEntity.getCarBondAmt() == null) {
//                        appAuctionEntity.getPrice().divide(new BigDecimal(100)).multiply(new BigDecimal(bail));
//                    } else {
//                        payAmount = appAuctionEntity.getCarBondAmt();
//                    }
//                    appAuctionEntity.setBail(payAmount);
//                }
//            }

            appAuctionEntity.setBail(appAuctionEntity.getCarBondAmt());
...
//            }
            ...
            BigDecimal mustPrice = appAuctionBidMapper.selectMustPrice(carId);
            if (mustPrice == null) {
                appAuctionEntity.setTopPrice(appAuctionEntity.getPrice());
            } else {
                appAuctionEntity.setTopPrice(mustPrice);
            }
...
//            }
            ...
            if (appAuctionEntity.getSourceType() == 2) { //二手车
                //修复前照片
                Wrapper<AppAuctionImgEntity> beforeRepair = new EntityWrapper<>();
                beforeRepair.eq("car_id", carId).eq("state", AppAuctionConstant.NINE);
                List<AppAuctionImgEntity> beforeRepairList = appAuctionImgService.selectList(beforeRepair);
                appAuctionEntity.setBeforeRepair(beforeRepairList);

                //修复后照片
                Wrapper<AppAuctionImgEntity> afterRepair = new EntityWrapper<>();
                afterRepair.eq("car_id", carId).eq("state", AppAuctionConstant.TEN);
                List<AppAuctionImgEntity> afterRepairList = appAuctionImgService.selectList(afterRepair);
                appAuctionEntity.setAfterRepair(afterRepairList);
            } else {
                //查图片
                Wrapper<AppAuctionImgEntity> wrapper2 = new EntityWrapper<>();
                wrapper2.eq("car_id", carId).eq("state", AppAuctionConstant.ONE);
            ...
            if (appAuctionEntity.getSourceType() == 1) { //拍卖车
            ...
            String carIdentificationNumber = appAuctionEntity.getCarIdentificationNumber();
            if (StringUtils.isNotEmpty(carIdentificationNumber) && carIdentificationNumber.length() > 6) {
                appAuctionEntity.setCarIdentificationNumber(carIdentificationNumber.substring(0, 6) + "***");
            }

            String licenseAddress = appAuctionEntity.getLicenseAddress();
            if (StringUtils.isNotEmpty(licenseAddress) && licenseAddress.length() > 2) {
                appAuctionEntity.setLicenseAddress(licenseAddress.substring(0, 3) + "***");
            }
...
//        wrapper.eq("car_id", id);
    ...
    public JSONObject getCarCount(Integer userId) {
        ...
        dto.setAuctionState(1);// 查询二手车在架数量
        ...
        images.put("oldCarNum", oldCarNum);
    ...
    private JSONObject a2Dto(AppAuctionEntity a) {
        ...
        object.put("source_type", a.getSourceType() == null ? 1 : a.getSourceType());
        ...
        if (appAuctionImgEntities.size() > 0) {
            for (AppAuctionImgEntity appAuctionImgEntity : appAuctionImgEntities) {
                if (a.getSourceType() == 1 && AppAuctionConstant.CAR_IMG_ONE.equals(appAuctionImgEntity.getState())) {
                    object.put("img", appAuctionImgEntity.getUrl());
                    break;
                } else if (a.getSourceType() == 2 && "10".equals(appAuctionImgEntity.getState())) {
                    object.put("img", appAuctionImgEntity.getUrl());
                    break;
                }
            }
    ...
    public JSONObject saveOrUpdate(UsedCarVo usedCarVo, Integer id) {
        JSONObject result = new JSONObject();
        if (usedCarVo.getId() == null) {
            // 新增
            EntityWrapper<AppAuctionEntity> wrapper1 = new EntityWrapper<>();
            wrapper1.eq("plate_no", usedCarVo.getPlateNo());
            AppAuctionEntity auction = selectOne(wrapper1);
            if (auction != null) {
                result.put("code", 4004);
                result.put("msg", "车辆已存在,请勿重复录入!");
            ...
            AppAuctionEntity entity = new AppAuctionEntity();
            entity.setUserId(id.toString());
            entity.setUpState(0);   // 未上架
            entity.setSourceType(2);
            entity.setCreateTime(new Date());
            BeanUtils.copyProperties(usedCarVo, entity);
            this.insert(entity);

            //保存修复前修复后图片
            List<AppAuctionImgEntity> imgEntities = new ArrayList<>();
            if (usedCarVo.getBeforeRepair() != null) {
                List<String> beforeRepair = usedCarVo.getBeforeRepair();
                if (usedCarVo.getBeforeRepair() != null && !beforeRepair.isEmpty()) {
                    imgEntities.addAll(getImgList(beforeRepair, "9", entity.getId()));
                }
            }

            List<String> afterRepair = usedCarVo.getAfterRepair();
            if (!afterRepair.isEmpty()) {
                imgEntities.addAll(getImgList(afterRepair, "10", entity.getId()));
            }
            appAuctionImgService.insertBatch(imgEntities);
            result.put("code", 200);
            result.put("msg", "保存成功");
            result.put("data", entity.getId());


            AppAuctionUpEntity appAuctionUpEntity = new AppAuctionUpEntity();
            appAuctionUpEntity.setCarId(entity.getId());
            appAuctionUpEntity.setUpdateTime(new Date());
            appAuctionUpEntity.setCreateTime(new Date());
            appAuctionUpEntity.setBrand(entity.getPlateNo());
            appAuctionUpMapper.insert(appAuctionUpEntity);

            return result;
        } else {
            //修改数据
            AppAuctionEntity auction = appAuctionService.selectById(usedCarVo.getId());
            if (auction.getCarState() >= AppAuctionConstant.THREE || auction.getCarState() == AppAuctionConstant.ONE) {
                result.put("code", 501);
                result.put("msg", "当前状态不能修改");
                return result;
            }
            //修改不能修改状态
            BeanUtils.copyProperties(usedCarVo, auction);
            auction.setUpdateTime(new Date());
            appAuctionMapper.updateById(auction);

            AppAuctionUpEntity appAuctionUpParamer = new AppAuctionUpEntity();
            appAuctionUpParamer.setCarId(auction.getId());

            AppAuctionUpEntity appAuctionUp = appAuctionUpMapper.selectOne(appAuctionUpParamer);
            appAuctionUp.setBrand(auction.getPlateNo());
            appAuctionUpMapper.updateById(appAuctionUp);
            ...
            if (usedCarVo.getBeforeRepair() != null && !beforeRepair.isEmpty()) {
                //删除数据
                EntityWrapper<AppAuctionImgEntity> tWrapper = new EntityWrapper<>();
                tWrapper.eq("car_id", auction.getId());
                tWrapper.eq("state", "9");
                appAuctionImgService.delete(tWrapper);
                imgEntities.addAll(getImgList(beforeRepair, "9", auction.getId()));
            }
            List<String> afterRepair = usedCarVo.getAfterRepair();
            if (!afterRepair.isEmpty()) {
                EntityWrapper<AppAuctionImgEntity> tWrapper = new EntityWrapper<>();
                tWrapper.eq("car_id", auction.getId());
                tWrapper.eq("state", "10");
                appAuctionImgService.delete(tWrapper);
                imgEntities.addAll(getImgList(afterRepair, "10", auction.getId()));
            }
            appAuctionImgService.insertBatch(imgEntities);
            result.put("code", 200);
            result.put("msg", "修改成功");
            result.put("data", auction.getId());
            return result;
        }
    }

    private List<AppAuctionImgEntity> getImgList(List<String> imgs, String status, Long id) {
        List<AppAuctionImgEntity> imgEntities = new ArrayList<>();
        for (int i = 0; i < imgs.size(); i++) {
            AppAuctionImgEntity img = new AppAuctionImgEntity();
            img.setCarId(id);
            img.setIndex(i + 1);
    ...
    public JSONObject findListByUsedCar(Map<String, Object> params, Integer userId) {
        List<UsedCarListVo> list = appAuctionMapper.findListByUsedCar(params);
        list.stream().forEach(x -> {
            String label = "";
            if (x.getRegisterDate() != null) {
                label += DateUtils.formateDate(x.getRegisterDate().getTime(), "yyyy") + "年 ";
            }

            if (x.getMileage() != null) {
                BigDecimal decimal = new BigDecimal(x.getMileage()).divide(new BigDecimal("10000")).setScale(2, BigDecimal.ROUND_HALF_UP);
                if (new BigDecimal(x.getMileage()).compareTo(new BigDecimal("1000")) < 0) {
                    //小于1000公里不转换
                    label += x.getMileage() + "公里";
                }else {
                    label += decimal + "万公里";
                }
            }

            if (x.getFirstAmount() != null) {
                label += " 首付" + x.getFirstAmount() + "万";
            }

            x.setLabel(label);

            String url = appAuctionImgService.findImgByRepair(x.getCarId());
            x.setUrl(url);
            if (userId == null) {
                x.setCollect(0);
            } else {
                AppAuctionCollectEntity collect = collectMapper.isCollect(Long.valueOf(userId), x.getCarId());
                x.setCollect(collect == null ? 0 : 1);
            }
        });
    ...
    public JSONObject findUsedCarDetail(Long carId) {
        //查询车辆详情
        JSONObject result = new JSONObject();
        AppAuctionEntity appAuctionEntity = this.selectById(carId);
        if (appAuctionEntity == null) {
            result.put("code", 407);
            result.put("msg", "参数有问题");
            return result;
        }
        UsedCarVo carVo = new UsedCarVo();
        carVo.setId(appAuctionEntity.getId().toString());
        BeanUtils.copyProperties(appAuctionEntity, carVo);
        //查询图片
        EntityWrapper<AppAuctionImgEntity> tWrapper = new EntityWrapper<>();
        tWrapper.eq("car_id", carId);
        tWrapper.eq("state", "9");
        List<AppAuctionImgEntity> img9 = appAuctionImgService.selectList(tWrapper);
        List<String> collect = img9.stream().map(AppAuctionImgEntity::getUrl).collect(Collectors.toList());
        carVo.setBeforeRepair(collect);

        EntityWrapper<AppAuctionImgEntity> tWrapper10 = new EntityWrapper<>();
        tWrapper10.eq("car_id", carId);
        tWrapper10.eq("state", "10");
        List<AppAuctionImgEntity> img10 = appAuctionImgService.selectList(tWrapper10);
        List<String> collect10 = img10.stream().map(AppAuctionImgEntity::getUrl).collect(Collectors.toList());
        carVo.setAfterRepair(collect10);

        AppAuctionUpEntity appAuctionUpParamer = new AppAuctionUpEntity();
        appAuctionUpParamer.setCarId(carId);
        AppAuctionUpEntity appAuctionUp = appAuctionUpMapper.selectOne(appAuctionUpParamer);
        carVo.setExplain(appAuctionUp.getExplain());

        carVo.setBeginTime(appAuctionUp.getBeginTime() != null ? appAuctionUp.getBeginTime().getTime() : null);
        carVo.setEndTime(appAuctionUp.getEndTime() != null ? appAuctionUp.getEndTime().getTime() : null);
        carVo.setCarBondAmt(appAuctionEntity.getCarBondAmt());
        carVo.setCarServiceAmt(appAuctionEntity.getCarServiceAmt());
    ...
    public JSONObject findListHotByUsedCar(Map<String, Object> params, Integer userId) {

        List<UsedCarListVo> list = appAuctionMapper.findListHotByUsedCar(params);
        list.stream().forEach(x -> {
            String label = "";
            if (x.getRegisterDate() != null) {
                label += DateUtils.formateDate(x.getRegisterDate().getTime(), "yyyy") + "年 ";
