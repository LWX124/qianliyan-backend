...
----------------------------------- 2023.05.17添加数据库字段 -----------------------------
ALTER TABLE `icars`.`app_auction`
ADD COLUMN `car_service_amt` decimal(10, 2)  NULL COMMENT '车辆服务费（元）' AFTER `car_state`;

ALTER TABLE app_auction ADD license_address varchar(255) DEFAULT NULL COMMENT '二手车——行驶证地址' AFTER car_bond_amt;

ALTER TABLE app_auction ADD car_identification_number varchar(255) DEFAULT NULL COMMENT '二手车——车辆识别码' AFTER license_address;

ALTER TABLE app_auction ADD source_type int(20) DEFAULT NULL COMMENT '二手车——信息来源默认是事故车，2.二手车' AFTER car_identification_number;

ALTER TABLE app_auction ADD car_introduction VARCHAR(200) DEFAULT NULL COMMENT '二手车——车辆介绍' AFTER source_type;



ALTER TABLE `icars_test`.`app_auction`
ADD COLUMN `car_bond_amt` int(10) NULL COMMENT '车辆保证金（元）' AFTER `car_state`;
ALTER TABLE `icars_test`.`app_auction`
MODIFY COLUMN `car_bond_amt` decimal(10, 2) NULL DEFAULT NULL COMMENT '车辆保证金（元）' AFTER `car_state`;

ALTER TABLE `icars_test`.`app_auction`
MODIFY COLUMN `car_service_amt` decimal(10, 2) NULL DEFAULT NULL COMMENT '车辆服务费（元）' AFTER `car_bond_amt`;

update app_auction set source_type=1 where source_type is n

ALTER TABLE `icars_test`.`app_auction`  
MODIFY COLUMN `source_type` int(20) NULL DEFAULT 1 COMMENT '二手车——信息来源默认是事故车，1.事故车 2.二手车' AFTER `car_source_type`;

update app_auction set car_bond_amt=price * 0.05 where car_bond_amt is null;

update app_auction set car_service_amt=price * 0.05 where car_service_amt is null;

ALTER TABLE app_auction ADD luxury_car_price decimal(10, 2) DEFAULT NULL COMMENT '二手车——精品车价格' AFTER car_introduction;

-- 2023.6.12---添加字段----------------------

ALTER TABLE app_auction ADD first_amount decimal(10, 2) DEFAULT NULL COMMENT '二手车——首付金额' AFTER luxury_car_price;
------ 2023.7.2 外网已执行 -------------------------