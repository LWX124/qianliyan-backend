...
public class AppAuctionBidService extends ServiceImpl<AppAuctionBidMapper, AppAuctionBidEntity> implements IService<AppAuctionBidEntity> {
    ...
    public JSONObject bidLog(Integer userId, AppAuctionBidEntity appAuctionBidEntity) {
        ...
        //同一时间对同一个车只能有一个用户出价
        String lockKey = RedisConstant.AUCTION_BID_LOCK + appAuctionBidEntity.getCarId();
        ...
        try {
            ...
            //记录出价,并记录有效出价车辆次
            appAuctionBidEntity.setBidTime(new Date().getTime());
            appAuctionBidEntity.setCreateTime(new Date());
            appAuctionBidEntity.setUserId(Long.valueOf(userId));
            appAuctionBidEntity.setUserName(appUserEntity.getName());
            ...
            //内部用户直接出价
            if (appUserEntity.getIsInner() == 1) {
                senLongMessage(appAuctionBidEntity, appAuctionUpEntity);
                result.put("code", 200);
                result.put("msg", "出价成功!");
                return result;
            }
    ...
    //发送长连接  记录最高出价  记录内部或者正常客户的出价信息
    private void senLongMessage(AppAuctionBidEntity appAuctionBidEntity, AppAuctionUpEntity appAuctionUpEntity) {
        ...
        redisTemplate.opsForSet().add(AuctionConstant.KEY_AUCTION_BID_LIST + appAuctionBidEntity.getCarId(), JSONObject.toJSONString(appAuctionBidEntity));
    ...
    public JSONObject bidRecord(JSONObject result, Long carId) {
        Set members = redisTemplate.opsForSet().members(AuctionConstant.KEY_AUCTION_BID_LIST + carId);

        List<AppAuctionBidEntity> collect = (List<AppAuctionBidEntity>) members.stream().map(x -> {
            JSONObject jsonObject = JSONObject.parseObject(String.valueOf(x));
            AppAuctionBidEntity appAuctionBidEntity = jsonObject.toJavaObject(AppAuctionBidEntity.class);
            return appAuctionBidEntity;
        }).sorted(Comparator.comparing(AppAuctionBidEntity::getBidTime).reversed()).collect(Collectors.toList());
