...
public class JedisPoolUtil {
    ...
    private static final String ACTIVE_PATH = "application.properties";
    private static final String ACTIVE_LOCAL_PATH = "application-local.properties";
    private static final String ACTIVE_TEST_PATH = "application-test.properties";
    private static final String ACTIVE_PROD_PATH = "application-prod.properties";
    ...
    private static void init() throws IOException {
        ...
        InputStream input = JedisPoolUtil.class.getClassLoader().getResourceAsStream(PROPERTIES_PATH);
        InputStream active = JedisPoolUtil.class.getClassLoader().getResourceAsStream(ACTIVE_PATH);
        InputStream localInput = null;
        InputStream testInput = null;
        InputStream prodInput = null;
        try {
            //基本配置
            //找到当前环境
            Properties activePro = new Properties();
            p.load(input);
            activePro.load(active);
            //不同环境读取不同的 redis密码
            if(activePro.getProperty("spring.profiles.active").equals("local")){
                localInput = JedisPoolUtil.class.getClassLoader().getResourceAsStream(ACTIVE_LOCAL_PATH);
                authPro.load(localInput);
            }else if(activePro.getProperty("spring.profiles.active").equals("test")){
                testInput = JedisPoolUtil.class.getClassLoader().getResourceAsStream(ACTIVE_TEST_PATH);
                authPro.load(testInput);
            }else if(activePro.getProperty("spring.profiles.active").equals("prod")){
                prodInput = JedisPoolUtil.class.getClassLoader().getResourceAsStream(ACTIVE_PROD_PATH);
                authPro.load(prodInput);
            }
        }finally {
            if(input!=null){
                input.close();
...
//        String host = (p.getProperty("redis.host").trim() == null || "".equals(p.getProperty("redis.host").trim())) ? "localhost" : p.getProperty("redis.host");
        ...
        String auth = authPro.getProperty("redis.auth").trim();
        String host = authPro.getProperty("redis.host").trim();
