...
public class WxPayV2Util {
    ...
    public WxPayMpOrderResult createOrder(String appId, String mchId, String mchKey,
        ...
        if (StringUtils.isNotBlank(mchKey)) wxPayConfig.setMchKey(mchKey);
        wxPayConfig.setSignType(WxPayConstants.SignType.MD5);
        if (StringUtils.isNotBlank(mchKey)) wxPayConfig.setTradeType(tradeType);
        if (!Objects.equal(WxPayConstants.TradeType.NATIVE, tradeType)) wxPayConfig.setNotifyUrl(v2PayNotifyUrl);
        ...
        if (StringUtils.isNotBlank(openId)) wxPayUnifiedOrderRequest.setOpenid(openId);
        if (StringUtils.isNotBlank(productId)) wxPayUnifiedOrderRequest.setProductId(productId);
