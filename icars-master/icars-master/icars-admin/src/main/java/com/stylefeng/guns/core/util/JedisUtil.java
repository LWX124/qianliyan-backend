...
public class JedisUtil {
     ...
     * @return
     */
    public int set(String key, String value, long time) {
        if (isValueNull(key, value)) {
            return 0;
        }
        Jedis jedis = null;
        try {
            ...
            if (jedis.set(key, value, name, "EX", time).equalsIgnoreCase("ok")) {
                return 1;
            } else {
                return 0;
            }
     ...
     * @throws JedisDataException
     */
    public Long decrBy(String key, long integer) throws JedisDataException {
        if (isValueNull(key)) {
            return null;
        }
        Jedis jedis = null;
     ...
     * @return
     */
    public int delSet(String key, String... value) {
        if (isValueNull(key, value)) {
            return 0;
        }
        Jedis jedis = null;
     ...
     * @return
     */
    public Long geoDEL(String key, String eleName) {
        Jedis jedis = null;
        jedis = JedisPoolUtil.getJedis();

        try {
            String[] params = new String[]{key, eleName};
            int keyCount = 2;
            return (Long) jedis.eval("return redis.call('ZREM',KEYS[1],KEYS[2])", keyCount, params);
        } finally {
            JedisPoolUtil.closeJedis(jedis);
        }
    ...
    public void expire(String key, Long expires) {
        ...
        jedis = JedisPoolUtil.getJedis();
        try {
            jedis.expire(key, Math.toIntExact(expires));
        } finally {
            JedisPoolUtil.closeJedis(jedis);
        }
