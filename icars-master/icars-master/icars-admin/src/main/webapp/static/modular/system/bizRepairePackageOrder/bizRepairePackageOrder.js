 ...
 * 管理初始化
 */
var BizRepairePackageOrder = {
    id: "BizRepairePackageOrderTable",	//表格id
    seItem: null,		//选中的条目
    table: null,
    layerIndex: -1
};
 ...
 * 初始化表格的列
 */
BizRepairePackageOrder.initColumn = function () {
    return [
...
function showDetail(val){
    $('#repaireOrderDetail').empty();
    var details = val.split('#');
    details.forEach(function( val, index ) {
        $('#repaireOrderDetail').append('<p style="padding: 5px;">套餐商品'+(index + 1)+':&nbsp;&nbsp;'+val+'</p>')
    });
    layer.open({
        type: 1,
 ...
 * 点击弹出推送框 推送维修人员
 */
BizRepairePackageOrder.pushRepaireMan = function () {
    if (this.check()){
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/bizRepairePackageOrder/order_push'
        });
        this.layerIndex = index;
    }
};

BizRepairePackageOrder.stopMaintain = function (){
    window.parent.document.getElementById("repaire_claim").pause();
}

/**
 * 接车确认
 */
BizRepairePackageOrder.recept = function () {
    if (this.check()) {
        ...
        var ajax = new $ax(Feng.ctxPath + "/bizRepairePackageOrder/order/accept", function (data) {
            Feng.success("操作成功!");
            BizRepairePackageOrder.table.refresh();
        }, function (data) {
 ...
 * 取消保养单
 */
BizRepairePackageOrder.cancel = function () {
    if (this.check()) {
        var orderId = this.seItem.id;
        var orderStatus = this.seItem.orderStatus;
        if (orderStatus == '已取消') {
        ...
        var ajax = new $ax(Feng.ctxPath + "/bizRepairePackageOrder/order/cancel", function (data) {
            Feng.success("操作成功!");
            BizRepairePackageOrder.table.refresh();
        }, function (data) {
            Feng.error("操作失败!" + data.responseJSON.message + "!");
 ...
 * 取消保养单
 */
BizRepairePackageOrder.finish = function () {
    if (this.check()) {
        var orderId = this.seItem.id;
        var orderStatus = this.seItem.orderStatus;
        if (orderStatus == '已完成') {
        ...
        var ajax = new $ax(Feng.ctxPath + "/bizRepairePackageOrder/order/finish", function (data) {
            Feng.success("操作成功!");
            BizRepairePackageOrder.table.refresh();
        }, function (data) {
            Feng.error("操作失败!" + data.responseJSON.message + "!");
 ...
 * 检查是否选中
 */
BizRepairePackageOrder.check = function () {
    var selected = $('#' + this.id).bootstrapTable('getSelections');
    if(selected.length == 0){
        Feng.info("请先选中表格中的某一记录！");
        return false;
    }else{
        BizRepairePackageOrder.seItem = selected[0];
        return true;
    }
};
 ...
 * 点击添加
 */
BizRepairePackageOrder.openAddBizRepairePackageOrder = function () {
    var index = layer.open({
        type: 2,
        title: '添加',
        area: ['800px', '420px'], //宽高
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/bizRepairePackageOrder/bizRepairePackageOrder_add'
    });
    this.layerIndex = index;
};
 ...
 * 打开查看详情
 */
BizRepairePackageOrder.openBizRepairePackageOrderDetail = function () {
    if (this.check()) {
        var index = layer.open({
            type: 2,
            title: '详情',
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/bizRepairePackageOrder/bizRepairePackageOrder_update/' + BizRepairePackageOrder.seItem.id
        });
        this.layerIndex = index;
    }
};
 ...
 * 删除
 */
BizRepairePackageOrder.delete = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/bizRepairePackageOrder/delete", function (data) {
            Feng.success("删除成功!");
            BizRepairePackageOrder.table.refresh();
        }, function (data) {
            Feng.error("删除失败!" + data.responseJSON.message + "!");
        });
        ajax.set("bizRepairePackageOrderId",this.seItem.id);
        ajax.start();
    }
};
 ...
 * 查询列表
 */
BizRepairePackageOrder.search = function () {
    var queryData = {};
    ...
    BizRepairePackageOrder.table.refresh({query: queryData});
};

BizRepairePackageOrder.resetSearch = function () {
    $("#condition").val("");
    $("#createStartTime").val("");
    $("#orderStatus").val("");
    BizRepairePackageOrder.search();
}
$(function () {
    var defaultColunms = BizRepairePackageOrder.initColumn();
    var table = new BSTable(BizRepairePackageOrder.id, "/bizRepairePackageOrder/list", defaultColunms);
    table.setPaginationType("server");
    BizRepairePackageOrder.table = table.init();
});
