...
//            }
            ...
            Integer withDraws = withdrawCashService.selectCount(new EntityWrapper<AppAuctionWithdrawCashEntity>().where("state=0").or("state=2").andNew().eq("user_id", withdrawCash.getUserId()));
            ...
            //查询 一口价未完结订单数量
            Integer onePriceOrderCount = appAuctionOnePriceCarLogMapper.countByUserId(withdrawCash.getUserId());
            if (onePriceOrderCount > 0) {
                result.put("code", 538);
                result.put("msg", "一口价车辆订单状态未完结,请勿提现!");
                return result;
            }
